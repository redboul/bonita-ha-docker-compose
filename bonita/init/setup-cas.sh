...
echo 'BonitaAuthentication-1 { \
  org.jasig.cas.client.jaas.CasLoginModule required \
    ...
    cacheTimeout="480"; \
};' > ${BONITA_PATH}/BonitaBPMSubscription-${BONITA_VERSION}-Tomcat-${TOMCAT_VERSION}/bin/jaas-standard.cfg
...
sed -i 's/=authenticationService/=jaasAuthenticationService/g' ${BONITA_HOME_COMMON_PATH}/engine-server/conf/tenants/template/bonita-tenant-sp-custom.properties
sed -i 's/=authenticationService/=jaasAuthenticationService/g' ${BONITA_HOME_COMMON_PATH}/engine-server/conf/tenants/1/bonita-tenant-sp-custom.properties
